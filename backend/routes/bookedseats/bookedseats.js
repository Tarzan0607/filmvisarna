...
router.get('/', async (req, res) => {
    ...
    if (bookedSeats.length === 0) return res.json({message: 'failed', response: []}).status(403), await pool.release();

    if (!bookedSeats[0]?.seat_id) return res.json({message: 'failed', response: []}).status(403), await pool.release();
